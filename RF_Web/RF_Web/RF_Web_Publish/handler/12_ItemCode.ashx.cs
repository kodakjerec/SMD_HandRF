    ...
    public class ItemCode : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            { 
                ...
                case "QueryTest":
                    QueryTest(context); break;
                case "QuerySunDay":
                    QuerySunDay(context);break;
                default:
                    break;
            }
        ...
        //測試
        private void QueryTest(HttpContext context)
        {
            string return_str = "";
            Hashtable ht = new Hashtable();
            Hashtable ht2 = new Hashtable();
            DataTable dt = dbIO.SqlSp("DCStest", "Anita_test", ht, ref ht2).Tables[0];
        ...
        //取太陽日
        private void QuerySunDay(HttpContext context)
        {
            string return_str = "";
            Hashtable ht = new Hashtable();

            DataTable dt = dbIO.SqlQuery("DCStest", "SELECT DATENAME(dayofyear, getdate()) AS 'SunDay'", ht).Tables[0];
        ...
        private void JOBID45(HttpContext context)
        {
            string return_str = "";
            Hashtable ht = new Hashtable();
            //ht.Add("@JOB_ID", 45); 已獨立新的SP，不須加JOB_ID
            ht.Add("@ID", context.Request.QueryString["IDNo"]);
            ht.Add("@ITEM_HOID", context.Request.QueryString["HOID"]);
            ht.Add("@USER_ID", context.Request.QueryString["USER_ID"]);

            Hashtable ht2 = new Hashtable();
            
            DataTable dt = dbIO.SqlSp("DCStest", "[id.spDCS_ID_LINE]", ht, ref ht2).Tables[0];//spactDCS_ID_LINE
            return_str = "{\"total\": " + dt.Rows.Count.ToString() + ", "
                        + " \"rows\":" + JSON.JSONconvert.DataTableToJSONstr(dt)
                        + "}";
